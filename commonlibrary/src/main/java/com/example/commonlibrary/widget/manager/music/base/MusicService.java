...
import com.example.commonlibrary.widget.manager.LocalInfoProvider;
import com.example.commonlibrary.widget.manager.music.MusicPlayerManager;
...
//                    LockScreenActivity.start(getBaseContext());
    ...
    private void updateMediaSession(PlayStateEvent state) {
        ...
        if (state.getPlayState() == DefaultVideoPlayer.
            ...
            if (musicPlayBean.getIsLocal()) {
                refreshMediaSession(playState, BitmapFactory.decodeResource(getResources(), R.drawable.ic_default_album_cover));
            } else {
    ...
    private void refreshMediaSession(int playState, Bitmap resource) {
        if (resource == null) {
            resource = BitmapFactory.decodeResource(getResources(), R.drawable.ic_default_album_cover);
        }
        MusicPlayBean musicPlayBean = MusicPlayerManager.getInstance().getMusicPlayBean();
    ...
    private void updateNotification() {
        ...
        builder.setSmallIcon(R.drawable.ic_default_album_cover)
    ...
    private RemoteViews getContentView() {
        if (remoteViews == null) {
            ...
            if (musicPlayBean != null) {
                ...
                if (musicPlayBean.getIsLocal()) {
                    remoteViews.setImageViewBitmap(R.id.iv_notification_icon, LocalInfoProvider.getAlbumsBitmap(musicPlayBean.getAlbumId()));
                } else {
                    Glide.with(BaseApplication.getInstance())
    ...
    private PendingIntent getContentIntent() {
        return null;
//        return PendingIntent.getActivity(this, 0, new Intent(this, HomeActivity.class), 0);
    }
