package com.example.commonlibrary.widget.manager.music;
...
import com.example.commonlibrary.widget.manager.music.base.IMusicPlayer;
...
import com.example.commonlibrary.widget.manager.music.base.PlayData;
import com.example.commonlibrary.widget.manager.music.base.PlayStateEvent;
...
public class MusicPlayerManager implements IMusicPlayer, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnBufferingUpdateListener, MediaPlayer.OnErrorListener {
    ...
    public List<MusicPlayBean> getplayList(){
        if (mPlayData != null) {
            return mPlayData.getPlayList();
        }
        return null;
    }
    ...
    public void play(List<MusicPlayBean> musicPlayBeans, int position, long seekPosition) {
        ...
        BaseApplication.getAppComponent()
                ...
                .putString(MusicUtil.RECENT_SONG_URL_LIST, BaseApplication
...
//            App.getMainComponent().getMainRepositoryManager().getDaoSession().getMusicPlayBeanDao()
//                    .update(mMusicPlayBean);
        ...
        if (mMusicPlayBean != null) {
            if (mMediaPlayer != null) {
                ...
                BaseApplication
                        ...
                        .edit().putLong(MusicUtil.SEEK, mMediaPlayer.getCurrentPosition()).apply();
                mMediaPlayer.reset();
            }
        }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        BaseApplication.getAppComponent().getSharedPreferences()
                .edit().putInt(MusicUtil.MUSIC_POSITION, mPlayData.getPosition()).apply();
        RxBusManager.getInstance().post(new PlayStateEvent(mState));
        mMediaPlayer.start();
        if (seekPosition != 0) {
            ...
            seekPosition = 0;
            BaseApplication.getAppComponent().getSharedPreferences()
                    .edit().putLong(MusicUtil.SEEK, seekPosition).apply();
        }
        mState = PLAY_STATE_PLAYING;
        RxBusManager.getInstance().post(new PlayStateEvent(mState));
    }
